...
for (let n=0; n<=3; n++){
    ...
    selectNumber.push(list[select]); //선택된 숫자를 넣음  
    list.splice(select, 1);   //중복되지않도록 넣은거 삭제
}
...
check.addEventListener('click', ()=>{
    ...
    if (value.length === 4){
        if (selectNumber.join('') === value) { //join은 배열을 문자열로 만들어줌
            ...
            let ball = 0;
            for (const [s_Keys, s_Number] of selectNumber.entries()){ //entries는 [key,value]를 같이 반환
                for (const [v_Keys, v_String] of valueArray.entries()){ 
